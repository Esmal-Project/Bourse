  ...
  async fetch(request,env){
    ...
    try{
      ...
      if(url.pathname==="/tsetmc/quote"){
        ...
        const res=await fetchUpstream("https://cdn.tsetmc.com/api/ClosingPrice/GetClosingPriceInfo/"+encodeURIComponent(insCode));const text=await res.text();if(!res.ok)return json({error:"TSETMC Quote upstream failed",status:res.status},502);return json({fetchedAt:new Date().toISOString(),source:"TSETMC Quote",payload:JSON.parse(text)});
      }
      if(url.pathname==="/tsetmc/orderbook"){
        const insCode=must(url.searchParams.get("insCode"),"insCode");
        const res=await fetchUpstream("https://cdn.tsetmc.com/api/BestLimits/"+encodeURIComponent(insCode));const text=await res.text();if(!res.ok)return json({error:"TSETMC Orderbook upstream failed",status:res.status},502);return json({fetchedAt:new Date().toISOString(),source:"TSETMC Orderbook",payload:JSON.parse(text)});
      }
      if(url.pathname==="/tsetmc/client-type"){
        const insCode=must(url.searchParams.get("insCode"),"insCode");
        const res=await fetchUpstream("https://cdn.tsetmc.com/api/ClientType/GetClientType/"+encodeURIComponent(insCode)+"/1/0");const text=await res.text();if(!res.ok)return json({error:"TSETMC ClientType upstream failed",status:res.status},502);return json({fetchedAt:new Date().toISOString(),source:"TSETMC ClientType",payload:JSON.parse(text)});
      }
      if(url.pathname==="/tsetmc/market-watch"){
        ...
        if(live?.Items?.length||live?.items?.length||Array.isArray(live)){
          return json({fetchedAt:new Date().toISOString(),source:"TSETMC MarketWatchCash",payload:live});
        }
