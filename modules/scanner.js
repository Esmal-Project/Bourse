...
function normalizeMarketWatch(raw){
  ...
  return rows.map(r=>{
    const pl=scanNum(r?.pdv??r?.pl??r?.pDrCotVal??r?.lastprice??r?.lastPrice??r?.pcl);
    ...
    const change=explicit??(scanNum(r?.pc)!=null&&py?scanNum(r?.pc)/py*100:(pl!=null&&py?((pl-py)/py)*100:null));
    return {
      ...
      last:pl,
      first:scanNum(r?.pf??r?.pFirst??r?.firstPrice),
      close:scanNum(r?.pcl??r?.pClosing??r?.closingprice??r?.closingPrice),
      yesterday:py,
      change,
